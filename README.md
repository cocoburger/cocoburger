<div align="center">

# Hi, I'm cocoburger 👋

<p><strong>Full-stack Web & App Developer</strong></p>

I build web and mobile experiences with TypeScript, React, Next.js, Vue, Nuxt, and React Native.

<a href="https://cocoburger.github.io/"><img src="https://img.shields.io/badge/Tech_Blog-181717?style=flat-square&logo=githubpages&logoColor=white" alt="Technical blog" /></a>
<a href="https://www.linkedin.com/in/burgerbum"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
<a href="mailto:fsm15@naver.com"><img src="https://img.shields.io/badge/Email-333333?style=flat-square" alt="Email" /></a>

</div>

## About Me

- I develop web applications with React, Next.js, Vue, and Nuxt, and mobile applications with React Native.
- I work across backend and data layers with NestJS, Firebase, and GraphQL.
- I deploy and operate applications on AWS and Oracle Cloud Infrastructure.
- I care about clear UI flows, maintainable architecture, and automated testing.
- I turn the problems I solve and the lessons I learn into practical technical writing.

## Tech Stack

### Web & Mobile

<img src="https://skillicons.dev/icons?i=ts,js,react,nextjs,vue,nuxtjs&theme=light" alt="TypeScript, JavaScript, React, Next.js, Vue, and Nuxt" />

![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB)

### Backend & Data

<img src="https://skillicons.dev/icons?i=nestjs,firebase,graphql&theme=light" alt="NestJS, Firebase, and GraphQL" />

### Cloud & Infrastructure

<img src="https://skillicons.dev/icons?i=aws&theme=light" alt="Amazon Web Services" />

![Oracle Cloud Infrastructure](https://img.shields.io/badge/Oracle_Cloud_Infrastructure-F80000?style=flat-square&logo=oracle&logoColor=white)

### UI & Testing

![Chakra UI](https://img.shields.io/badge/Chakra_UI-319795?style=flat-square&logo=chakraui&logoColor=white)
![Recoil](https://img.shields.io/badge/Recoil-3578E5?style=flat-square)
![Mock Service Worker](https://img.shields.io/badge/Mock_Service_Worker-FF6A33?style=flat-square&logo=mockserviceworker&logoColor=white)
![Jest](https://img.shields.io/badge/Jest-C21325?style=flat-square&logo=jest&logoColor=white)

### Workflow

<img src="https://skillicons.dev/icons?i=git,githubactions&theme=light" alt="Git and GitHub Actions" />

## Technical Writing

I write about frontend engineering, backend testing, UI architecture, and the trade-offs behind real-world implementation decisions. Articles are written in Korean.

- [Troubleshooting Next.js and CloudFront](https://cocoburger.github.io/posts/nextjs-cloudfront-cache-deep-dive/) — RSC caching, API uploads, and image cache invalidation.
- [UI Flow Specification: Practical Patterns](https://cocoburger.github.io/posts/ui-flow-spec-%EC%8B%A4%EC%A0%84-%ED%8C%A8%ED%84%B4/) — connecting UI specifications to state, code, and testing.
- [Getting Started with NestJS Testing](https://cocoburger.github.io/posts/NestJS-%ED%85%8C%EC%8A%A4%ED%8A%B8-%EC%B2%AB%EA%B1%B8%EC%9D%8C/) — unit, integration, and end-to-end testing fundamentals.

[Read more on my technical blog →](https://cocoburger.github.io/)

## Public GitHub Activity

[Browse my repositories and contribution history →](https://github.com/cocoburger)

## Let's Connect

- Technical blog: [cocoburger.github.io](https://cocoburger.github.io/)
- LinkedIn: [burgerbum](https://www.linkedin.com/in/burgerbum)
- Email: [fsm15@naver.com](mailto:fsm15@naver.com)
